...
int main(int argc, char *argv[])
{
	...
	fclose(fp);
/*	
	Check for any unwanted characters
	for (int i = 0; i < strlen(plaintext); i++)
	{
		if ((int)plaintext[i] < 65 && (int)plaintext[1] != 32 && (int)plaintext[i] != 10)
		{
	...
	{
		if ((int)key[i] < 65 && (int)key[1] != 32 && (int)key[i] != 10)
		{
			fprintf(stderr, "Invalid character found: %c\n", key[i]);
			exit(1);
		}	
...
*/
	...
	if (strcmp(buffer, ACCEPTED) == 0)
	{
		...
		char *temp = malloc(strlen(bigString)*sizeof(char));
		...
		*len = strlen(bigString);
		...
		{
			//error("CLIENT: ERROR attempting to connect to wrong service");
			printf("Password denied by the daemon\n");
			close(socketFD);
			exit(2);
		}
